...
-- prevents mod from appearing in script voice param even if mod is enabled, by blocking hook registration. Only works when loading mod from within script. Will prevent the mod from working when enabled from the system mod menu, however.
-- if norns.state.shortname == 'dreamsequence' then
  -- print('mod.lua detected ' ..  norns.state.shortname)
  ...
  local function add_player(cv, env)  -- confirmed OK
      local player = {
          ext = "_"..cv,
          count = 0,
          tuning = false,
      ...
      function player:add_params()
          -- params:add_group("nb_ds_crow_"..self.ext, "crow_ds "..cv.."/"..env, 12)
          params:add_group("nb_ds_crow_"..self.ext, "crow_ds "..cv, 12)
          params:add_option("nb_ds_crow_env_out"..self.ext, "env. out",{"off", 1, 2, 3, 4}, 1)
      ...
      function player:describe(note)
          return {
              -- name = "crow "..cv.."/"..env, -- don't know where this is used but it is important
              name = "crow "..cv,
              supports_bend = false,
              supports_slew = true,
      ...
      function player:tune()
        ...
        note_players["crow_ds "..cv] = player
  ...
  mod.hook.register("script_pre_init", "nb ds_crow pre init", function()  -- disabling this seems to prevent mod from remaining after switching from DS to another script
      -- add_player(1, 2)
      -- add_player(3, 4)
...
-- else
    -- script is not dreamsequence and mod is not initialized even if it's enabled in the mod menu
    -- print("Dreamsequence not detected")
-- end